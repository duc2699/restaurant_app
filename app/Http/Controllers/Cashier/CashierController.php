...
class CashierController extends Controller
{
    ...
    public function orderFood(Request $request)
    {
        ...
        $sale->total_price = $sale->total_price + ($request->quantity *
            $menu->price);
        $sale->save();
    ...
    public function getSaleDetailByTable($table_id)
    {
        $sale = Sale::where('table_id', $table_id)
            ->where('sale_status', 'unpaid')->first();
    ...
    public function getSaleDetails($sale_id)
    {
        ...
        $saleDetails = SaleDetail::where('sale_id', $sale_id)
            ->get();
        $html .= '<div class="table-responsive-sm" style="overflow-y:scroll;
        ...
        foreach ($saleDetails as $saleDetail) {
            ...
            $decreaseButton = '<button class="btn btn-light btn-sm 
            btn-decrease-quantity" disabled >-</button>';
            if ($saleDetail->quantity >1){
                $decreaseButton = '<button data-id="'.$saleDetail->id.'" class="btn btn-light btn-sm 
            ...
            $html .= '
                        <tr>
                            ...
                            <td>'.$decreaseButton.'
                            ' . $saleDetail->quantity . '
                            <button data-id="'.$saleDetail->id.'" class="btn btn-light btn-sm 
                            btn-increase-quantity" >+</button>
                            </td>
                            <td>' . $saleDetail->menu_price . '</td>
                            <td>' . ($saleDetail->menu_price * $saleDetail->quantity) . '</td>';
            if ($saleDetail->status == "noConfirm") {
                $showBtnPayment = false;
                $html .='<td> <a data-id="'.$saleDetail->id.'" 
                class="btn btn-danger btn-delete-saledetail">
        ...
        if ($showBtnPayment) {
            $html .= '<button data-id="' . $sale->id . '"
            data-totalAmount="'.$sale->total_price.'"
            class="btn btn-success 
            btn-block btn-payment"  data-toggle="modal"
             data-target="#exampleModal">
            Payment
            </button>';
        } else {
            $html .= '<button data-id="' . $sale->id . '"class="btn btn-warning btn-block btn-confirm-order">
            Confirm Order
            </button>';
        }
    ...
    public function showReceipt($saleID){
        ...
        return view('cashier.showReceipt')->with('sale',$sale)
